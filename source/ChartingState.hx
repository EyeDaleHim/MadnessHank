...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addSectionUI():Void
	{
		...
		stepperLength.value = _song.notes[daSection].lengthInSteps;
		...
		var copyButton:FlxButton = new FlxButton(10, 130, "Copy current section", function()
		{
			// copySection(Std.int(daSection));
			copiedSection = daSection;
		});

		var pasteSection:FlxButton = new FlxButton(10, 150, "Paste Copied", function()
		{
			copySection(Std.int(copiedSection));
		});

		var clearSectionButton:FlxButton = new FlxButton(10, 170, "Clear", clearSection);

		var swapSection:FlxButton = new FlxButton(10, 190, "Swap section", function()
		{
			for (i in 0..._song.notes[daSection].sectionNotes.length)
			{
				var note = _song.notes[daSection].sectionNotes[i];
				note[1] = (note[1] + 4) % 8;
				_song.notes[daSection].sectionNotes[i] = note;
				updateGrid();
			}
		});

		var cleanUp:FlxButton = new FlxButton(10, 210, "Clean Up Notes", cleanUpSection);
		...
		tab_group_section.add(pasteSection);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		if (id == FlxUICheckBox.CLICK_EVENT)
		{
			...
			switch (label)
			{
				case 'Must hit section':
					_song.notes[daSection].mustHitSection = check.checked;

					updateHeads();

				case 'Change BPM':
					_song.notes[daSection].changeBPM = check.checked;
					FlxG.log.add('changed bpm shit');
				case "Alt Animation":
					_song.notes[daSection].altAnim = check.checked;
			}
		}
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			{
				_song.notes[daSection].lengthInSteps = Std.int(nums.value);
				updateGrid();
			}
			...
			{
				_song.notes[daSection].bpm = Std.int(nums.value);
				updateGrid();
			}
		}
	...
	/* this function got owned LOL
		...
		{
			if (_song.notes[daSection].changeBPM)
				return _song.notes[daSection].lengthInSteps * (_song.notes[daSection].bpm / _song.bpm);
			else
				return _song.notes[daSection].lengthInSteps;
	}*/
	...
	override function update(elapsed:Float)
	{
		...
		strumLine.y = getYfromStrum((Conductor.songPosition - sectionStartTime()) % (Conductor.stepCrochet * _song.notes[daSection].lengthInSteps));

		if (strumLine.y < gridBG.y - 3)
		{
			if (daSection != 0)
				changeSection(daSection - 1, false);
			else
				FlxG.sound.music.time = 0;
		}

		if (curBeat % 4 == 0 && curStep >= 16 * (daSection + 1))
		{
			trace(curStep);
			trace((_song.notes[daSection].lengthInSteps) * (daSection + 1));
			trace('DUMBSHIT');

			if (_song.notes[daSection + 1] == null)
			{
				addSection();
			}
		...
		if (FlxG.mouse.justPressed)
		{
			...
			else
			{
				if (FlxG.mouse.x > gridBG.x
					...
					&& FlxG.mouse.y < gridBG.y + (GRID_SIZE * _song.notes[daSection].lengthInSteps))
				{
		...
		if (FlxG.keys.justPressed.RIGHT || FlxG.keys.justPressed.D)
			changeSection(daSection + shiftThing);
	...
	function cleanUpSection()
	{
		...
		for (i in 0...2)
		{
			curRenderedNotes.forEach(function(note:Note)
			{
				if (note.y < gridBG.y || note.y > gridBG.y)
					deleteNote(note);
			});
		}
	}
	...
	{
		/*
			var daSec = FlxMath.maxInt(daSection, sectionNum);
		...
		var daSec = FlxMath.maxInt(copiedSection, sectionNum);
	...
	function updateGrid():Void
	{
		...
		var sectionInfo:Array<Dynamic> = _song.notes[daSection].sectionNotes;
		...
		for (i in sectionInfo)
		{
			...
			note.y = Math.floor(getYfromStrum((daStrumTime - sectionStartTime()) % (Conductor.stepCrochet * _song.notes[daSection].lengthInSteps)));
	...
	function selectNote(note:Note):Void
	{
		...
		for (i in _song.notes[daSection].sectionNotes)
		{
			...
			{
				curSelectedNote = _song.notes[daSection].sectionNotes[swagNum];
			}
	...
	private function addNote():Void
	{
		...
		_song.notes[daSection].sectionNotes.push([noteStrum, noteData, noteSus]);

		curSelectedNote = _song.notes[daSection].sectionNotes[_song.notes[daSection].sectionNotes.length - 1];

		if (FlxG.keys.pressed.ONE)
		{
			_song.notes[daSection].sectionNotes.push([noteStrum, (noteData + 4) % 8, noteSus]);
		}
